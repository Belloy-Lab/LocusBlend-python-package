...
``COLOR_MAPPING`` is moved verbatim from ``app.2.8.12.py`` and remains the
authoritative mapping for three-index ``group_code`` values (the app's values
are not regenerated, simplified or reordered). Keep it unchanged unless the
baseline itself changes.
"""
